...
export type BookingWeeklySchedule = Partial<
  Record<Weekday, BookingTimeWindow[]>
>;
...
export interface KnowledgeCrawlJob {
  ...
  /**
   * Storage/grouping namespace. Not "where user uploaded from".
   */
  domain: string;
...
export async function crawlBotDomain(
  ...
  return authFetchJson<CrawlEstimateResponse | CrawlStartResponse>(
...
export async function getBotCrawlHistory(
  ...
  return authFetchJson<KnowledgeCrawlHistoryResponse>(
    `/bots/${encodeURIComponent(botId)}/knowledge/crawl-history?page=${encodeURIComponent(
      String(page)
    )}`
  );
}
...
export async function fetchBotConversations(
  ...
  return authFetchJson<PaginatedConversations>(
    `/conversations/bots/${encodeURIComponent(botId)}${query}`
  );
}
...
// NEW: evaluate a conversation (manual trigger from UI)
...
export async function bulkEvaluateConversationsApi(
  ...
  const res = await authFetchJson<{ items: BulkConversationEvalItem[] }>(
...
// ✅ UPDATED: accept optional PIN and send only if provided
...
export async function fetchWhatsappTemplates(
  ...
  const url =
    `/bots/${encodeURIComponent(botId)}/whatsapp/templates` +
    (qs ? `?${qs}` : "");
...
export async function updateWhatsappTemplate(
  ...
  return authFetchJson<{ ok: boolean }>(
    `/bots/${encodeURIComponent(
      botId
    )}/whatsapp/templates/${encodeURIComponent(templateId)}`,
    {
      method: "PUT",
      body: JSON.stringify(payload)
    }
...
export async function fetchMetaLeads(
  ...
  const url =
    `/bots/${encodeURIComponent(botId)}/meta-leads` + (qs ? `?${qs}` : "");
